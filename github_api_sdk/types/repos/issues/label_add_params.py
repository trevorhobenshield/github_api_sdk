...
class Variant0(TypedDict, total=False):
    ...
    labels: list[str]
    """The names of the labels to add to the issue's existing labels.

    You can pass an empty array to remove all labels. Alternatively, you can pass a
    single label as a `string` or an `array` of labels directly, but GitHub
    recommends passing an object with the `labels` key. You can also replace all of
    the labels for an issue. For more information, see
...
LabelAddParams: TypeAlias = Union[Variant0, Variant1, Variant2, Variant3, Variant4]
