...
class CommentCreateParams(TypedDict, total=False):
    ...
    To find the ID of a review comment with
    ...
    The line of the blob in the pull request diff that the comment applies to. For a
    multi-line comment, the last line of the range that your comment applies to.
    """

    position: int
    """\\**\\**This parameter is closing down.

    Use `line` instead\\**\\**. The position in the diff where you want to add a review
    comment. Note this value is not the same as the line number in the file. The
    position value equals the number of lines down from the first "@@" hunk header
    in the file you want to add a comment. The line just below the "@@" line is
    ...
    The `start_line` is the first line in the pull request diff that your multi-line
    ...
    start_side: Literal["LEFT", "RIGHT", "side"]
    """**Required when using multi-line comments unless using `in_reply_to`**.
